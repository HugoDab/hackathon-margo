...
export const generateNewMarker = (
  { lat, lng, map }: { lat: number; lng: number; map: Map },
  title: string,
  description: string,
  markerColor: string,
  removable: boolean,
  ...
  const popUp = new Popup({ closeButton: true, anchor: "left" }).setHTML(
    `<div class="popup" style="color:black"><h2>${title}</h2><p>${
      operator ?? ""
    }</p><p>${description}</p><p>${bottleText}</p><p>${indoorText}</p><p style="font-style:italic">${
      city ?? ""
    }</p></div>`
  );

...
import {
  clearAllMarkers,
  generateNewMarker,
} from "@/components/Map/generateNewMarker";
...
export function showAllMarker(map: mapboxgl.Map, limit?: number) {
  ...
  const dist =
    ...
    distanceUsingLatLog(
      deg2rad(maxLat),
      deg2rad(map.getCenter().lat),
      deg2rad(maxLong),
      deg2rad(map.getCenter().lng)
    );

  axios
    .get(
      `http://localhost:8080/drinkingWater?long=${map.getCenter().lng}&lat=${
        map.getCenter().lat
    ...
    .then((data) => {
      ...
      for (const drinkingWaterPoint of drinkingWaterPointArray) {
        ...
        generateNewMarker(
          ...
          title,
          drinkingWaterPoint.description,
          color,
          true,
          drinkingWaterPoint.operator,
          drinkingWaterPoint.accepts_bottle,
          drinkingWaterPoint.indoor,
          drinkingWaterPoint.com_nom
        );
      }
    })
...
export const useMap = (container: React.RefObject<HTMLDivElement>) => {
    ...
    if (container.current) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          mapInitRef.current = initMap(container.current as HTMLDivElement, [
  ...
  const setCoords = (lat: number, long: number) => {
    setCurrentLat(lat);
    setCurrentLong(long);
  ...
  return { mapInitRef, currentLat, currentLong, setCoords };
};

function distanceUsingLatLog(
  lat1: number,
  lat2: number,
  long1: number,
  long2: number
